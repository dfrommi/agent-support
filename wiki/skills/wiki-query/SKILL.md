...
description: >
  ...
  Optionally save valuable answers back into the wiki as new pages. Use when the
  user asks a question about wiki content, says "what do we know about", "query
  the wiki", "search the wiki", "synthesize", "compare", or asks any question
  that could be answered from existing wiki pages. Also triggers on requests to
  create comparisons, analyses, or summaries from wiki content.
---
...
Start by reading `private/wiki/index.md` to identify candidate pages. Then read the most relevant pages.
...
```bash
grep -rl "search term" private/wiki/
```

When a question's answer is plausibly in raw material that hasn't been integrated yet, also search the insights corpus — insights often contain finer-grained detail than the wiki pages they fed into:
```bash
grep -rl "search term" private/insights/
```
If a relevant insight exists but isn't yet ingested into the wiki, mention this and suggest running `wiki-ingest` on it.

### 3. Synthesize the answer

Compose an answer grounded in wiki content. Key rules:

- **Cite your sources.** Reference wiki pages with `[[wikilinks]]` so the user can follow up. For claims traceable to raw sources, cite those too.
...
- **Synthesis pages** (type: synthesis) for cross-cutting analyses
- **Comparison pages** (type: comparison) for structured comparisons

If the user agrees, create the page following wiki conventions, update `private/wiki/index.md`, and append to `private/wiki/log.md`:
...
- **Suggest next steps.** After answering, suggest related questions the user might want to explore, or sources that could strengthen weak areas.
