...
- Avoid premature generalization. No abstractions for single-use code.
...
- `ast-grep` (`sg`): for structural code searches.
- `rip-grep` (`rg`): for free-text searches (faster `grep`).
- `fd`: for finding files (faster `find`).
