...
type RenderOption = QuestionOption & { isOther?: boolean; originalIndex?: number };
...
const QuestionOptionSchema = Type.Object({
    ...
    recommended: Type.Optional(Type.Boolean({ description: "Mark this option as the LLM's recommended choice" })),
});
    ...
    pi.registerTool({
        ...
        async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
            ...
            for (const q of questions) {
                ...
                if (recs.length > 1) {
                    return errorResult("Error: At most one option can be marked as recommended", questions);
                }
            }
            ...
            const result = await ctx.ui.custom<QuestionnaireResult>((tui, theme, _kb, done) => {
                ...
                function handleInput(data: string) {
                    ...
                    if (matchesKey(data, Key.enter) && q) {
                        ...
                        saveAnswer(q.id, opt.value, opt.label, false, opt.originalIndex ?? optionIndex + 1);
                        advanceAfterAnswer();
                        return;
                    }
                ...
                function render(width: number): string[] {
                    ...
                    function renderOptions() {
                        for (let i = 0; i < opts.length; i++) {
                            ...
                            const num = opt.originalIndex ?? i + 1;
                            const prefix = selected ? theme.fg("accent", "> ") : "  ";
                            const recSuffix = opt.recommended ? theme.fg("dim", " [recommended]") : "";
                            const label = `${num}. ${opt.label}${recSuffix}${isOther && inputMode ? " ✎" : ""}`;
