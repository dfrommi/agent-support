---
description: Create an implementation plan from the current conversation
argument-hint: '[focus]'
---
...
Output the plan in this conversation, not into a file unless explicitly asked to do so.
...
- Do not silently decide risky behavior. Never write a plan artifact while it still contains unresolved blocking questions. Ask first, then write.
- When you discover multiple valid approaches (degrees of freedom), you must use `ask_user_question` to resolve them before finalizing the plan. Do not merely list alternatives — the user must choose.
...
- **Degrees of freedom and decisions** — alternatives and user choices
