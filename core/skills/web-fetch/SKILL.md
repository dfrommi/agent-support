...
- `--multi=brave,exa,parallel` — federate several backends (rank-fused, deduped); does not support `site` filters
...
- `--backend github` adds star counts
`--regex` interprets the query as a regex
